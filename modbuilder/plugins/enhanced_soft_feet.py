...
OPTIONS = [
  { "name": "Soft Feet Percent", "type": int, "min": 30, "max": 90, "default": 20, "increment": 10 }
]

def format(options: dict) -> str:
  sound = options['soft_feet_percent']
  updated_value = 1.0 - sound / 100
  return f"Enhanced Soft Feet ({int(sound)}%, {round(updated_value,1)})"
